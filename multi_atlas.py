...
def atlas_reg(i, WDIR, fn_img_targ, coords_targ, atlas_set):

    # numeric tag for output files related to this atlas
    tag = str(i).zfill(2)
    print('Atlas' + tag + ' is processing')
    ...
    T = tf.similarity_tform(coords_targ,coords_atlas)
...
if __name__ == "__main__":
    ...
    #for i in range(0, len(atlas_set)):
    #    atlas_reg(i, WDIR, fn_img_targ, coords_targ, atlas_set)
    ...
    lock = mp.Lock()
    for i in range(0,len(atlas_set)):
        mp.Process(target=f,args=(lock, i)).start()
        '''
